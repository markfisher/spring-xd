...
/**
 * The {@link JobLaunchingGateway} is used to launch Batch Jobs. Internally it delegates to a
 * {@link JobLaunchingMessageHandler}.
 * 
...
public class JobLaunchingGateway extends AbstractReplyProducingMessageHandler {
	...
	/**
	 * Launches a Batch Job using the provided request {@link Message}. The payload of the {@link Message} <em>must</em>
	 * be an instance of {@link JobLaunchRequest}.
	 * 
	 * @param The provided requestMessage must not be null.
	 * @return Generally a {@link JobExecution} will always be returned. An exception ({@link MessageHandlingException})
	 *         will only be thrown if there is a failure to start the job. The cause of the exception will be a
