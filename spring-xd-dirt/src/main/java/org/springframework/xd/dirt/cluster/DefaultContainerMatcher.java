...
public class DefaultContainerMatcher implements ContainerMatcher, ApplicationContextAware {
	...
	private Collection<ContainerEligibilityFilter> containerEligibilities;
	...
	public Collection<Container> match(ModuleDescriptor moduleDescriptor,
		...
		List<Container> filteredContainers = applyEligibilityFilter(moduleDescriptor, candidates);
	...
	protected List<Container> applyEligibilityFilter(ModuleDescriptor moduleDescriptor, List<Container> candidates) {
		if (containerEligibilities != null) {
			for (ContainerEligibilityFilter eligibilityFilter : containerEligibilities) {
				candidates = eligibilityFilter.applyFilter(moduleDescriptor, candidates);
			}
		}
		return candidates;
	}
	...
	public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
		...
		this.containerEligibilities = appContext.getBeansOfType(ContainerEligibilityFilter.class).values();
	}
}
